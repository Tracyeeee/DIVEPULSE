 ...
 * @returns {string} 格式化的时间 HH:MM:SS
 */
...
/**
 * 解析倒计时字符串为秒数
 * @param {string} countdown - 倒计时字符串 HH:MM:SS
 * @returns {number} 总秒数
 */
export const parseCountdown = (countdown) => {
  if (typeof countdown !== 'string') return 0;
  
  const parts = countdown.split(':').map(p => parseInt(p, 10));
...
/**
 * 递减倒计时（带边界保护）
 * @param {string} current - 当前倒计时字符串
 * @returns {string} 递减后的倒计时
 */
export const decrementCountdown = (current) => {
  const total = parseCountdown(current);
  if (total <= 0) return '00:00:00';
  return formatCountdown(total - 1);
};
